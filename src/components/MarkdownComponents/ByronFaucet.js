...
const endpoint = 'https://faucet2.cardano-testnet.iohkdev.io/send-money'

const ByronFaucet = ({ content }) => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const result = await fetch(`${endpoint}/${values.address}`, {
        method: 'POST'
      })
        ...
        <Box marginTop={4} marginBottom={4}>
          ...
          {result.txid &&
            <Fragment>
              ...
              <p><strong><Link href={`https://cardano-explorer.cardano-testnet.iohkdev.io/tx/${result.txid}`}>{result.txid}</Link></strong></p>
            </Fragment>
          }
          <Box marginTop={2}>
...
ByronFaucet.propTypes = {
  content: PropTypes.object.isRequired
}

export default () => (
  <GlobalContentQuery
    render={content => <ByronFaucet content={content} />}
  />
)
